...
% 
% for i = 1 : m
%     sigma(i,2) = norm(Phi(i,:));
% end
% 
% tau = gamma / m / max(max(sigma));
% 
% sigma = (gamma./ sigma) / 2 / l;
% 
% 
% x = x0;
% 
...
%     
%     vtemp = (v(j)+sigma(j,2)*(Phi(j,:)*x-y(j))) / (1+sigma(j,2)*l);
% 
...
% save('Exp1_SVRG_Screening.mat','err_nested_SVRG_screening_hist');
% save('Exp1_SPDHG.mat','err_SPDHG_hist');